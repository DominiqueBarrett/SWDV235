...
public partial class Views_contact : System.Web.UI.Page
{
    ...
    protected void newsLetterSubmit_Click(object sender, EventArgs e)
    {
        ...
        if(!isFNValid || !isLNValid || !isEMValid || !isPNValid)
        {
            return;
        }
        //Call database
    ...
    private void StoreContact()
    {
        if (Page.IsValid)
        {
            ...
            // Create command
            comm = new SqlCommand(
               "EXECUTE sp_StoreContact @FirstName,@LastName,@EmailAddress,@PhoneNumber", conn);
            ...
            try
            {
                ...
                lblCompleteMessage.Text = "Thank you for contacting us! We will send a reply to: " + txtEmailAddress.Text + " soon!.";
            }
